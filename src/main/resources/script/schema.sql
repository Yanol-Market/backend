CREATE TABLE `users` (
    ...
    role           VARCHAR(50)      CHECK (role IN ('ROLE_GUEST', 'ROLE_USER', 'ROLE_ADMIN'))   COMMENT '역할',
    yanolja_id     VARCHAR(100)                                                                 COMMENT '야놀자 ID',
    deleted        BOOLEAN          NOT NULL                                                    COMMENT '삭제 여부',
    created_at     DATETIME                                                                     COMMENT '생성 일시',
    updated_at     DATETIME                                                                     COMMENT '수정 일시'
) COMMENT '사용자' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;

CREATE TABLE `product` (
    id                    BIGINT        AUTO_INCREMENT PRIMARY KEY                              COMMENT '상품 ID',
    user_id               BIGINT        NOT NULL                                                COMMENT '사용자 ID',
    reservation_id        BIGINT                                                                COMMENT '예약 ID',
    reservation_type      VARCHAR(50)   CHECK (reservation_type IN ('DAY_USE', 'STAY'))         COMMENT '예약 유형 확인',
    ...
    area_code             VARCHAR(50)   CHECK (area_code IN ('SEOUL', 'GYEONGGI', 'INCHEON',
                                                            'GANGWON', 'JEJU','DAEJEON',
                                                            'CHUNGBUK', 'CHUNGNAM', 'BUSAN',
                                                            'ULSAN','GYEONGNAM', 'DAEGU',
                                                            'GYEONGBUK', 'GWANGJU','JEONNAM',
                                                            'JEONBUK'))                         COMMENT '시도 코드',
...
) COMMENT '상품' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `wish_region` (
    ...
    updated_at DATETIME                                 COMMENT '수정 일시',
    FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
) COMMENT '관심지역' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
) COMMENT '찜' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `alert` (
    ...
    updated_at DATETIME                                     COMMENT '수정 일시',
    FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
) COMMENT '알림' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;

CREATE TABLE `alert_setting` (
    id         BIGINT     AUTO_INCREMENT PRIMARY KEY  COMMENT '알림 설정 ID',
    user_id    BIGINT     NOT NULL                    COMMENT '사용자 ID',
    marketing  BOOLEAN    NOT NULL                    COMMENT '마케팅 알림 설정 여부',
    created_at DATETIME                               COMMENT '생성 일시',
    updated_at DATETIME                               COMMENT '수정 일시',
    FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
) COMMENT '알림설정' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `nego` (
    ...
    FOREIGN KEY (`product_id`) REFERENCES `product` (`id`),
    FOREIGN KEY (`user_id`) REFERENCES `users` (`id`)
) COMMENT '네고' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;

CREATE TABLE `orders` (
    id                  BIGINT      AUTO_INCREMENT PRIMARY KEY                                          COMMENT '주문 ID',
    product_id          BIGINT      NOT NULL                                                            COMMENT '상품 ID',
    user_id             BIGINT      NOT NULL                                                            COMMENT '구매자 ID',
    `status`            VARCHAR(50) NOT NULL CHECK (status IN ('REQUEST_PAYMENT', 'WAITING_TRANSFER',
                                                             'COMPLETED_TRANSFER', 'FAIL', 'CANCEL')) COMMENT '주문 상태',
    price               INT         NOT NULL                                                            COMMENT '주문 가격',
    customer_view_check BOOLEAN     NOT NULL                                                            COMMENT '구매자 VIEW',
    created_at          DATETIME                                                                        COMMENT '생성 일시',
    updated_at          DATETIME                                                                        COMMENT '수정 일시',
    FOREIGN KEY (`product_id`) REFERENCES `product` (`id`)
) COMMENT '주문' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `payment` (
    ...
    `status`              VARCHAR(50)   NOT NULL CHECK (status IN ('READY', 'PAID',
                                                                  'CANCELLED', 'FAILED'))  COMMENT '결제 상태',
...
) COMMENT '결제' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `payment_cancel_detail` (
    ...
    created_at  DATETIME                                    COMMENT '생성 일시',
    updated_at  DATETIME                                    COMMENT '수정 일시'
) COMMENT '취소내역' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
) COMMENT '채팅' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE `chat_room` (
                             ...
                             `created_at` DATETIME NOT NULL COMMENT '생성 일시',
                             `updated_at` DATETIME NOT NULL COMMENT '수정 일시'
) COMMENT '채팅방' ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE reservation (
    ...
    area_code             ENUM ('SEOUL', 'GYEONGGI', 'INCHEON', 'GANGWON',
                             'JEJU', 'DAEJEON', 'CHUNGBUK', 'CHUNGNAM',
                             'BUSAN', 'ULSAN', 'GYEONGNAM', 'DAEGU',
                             'GYEONGBUK', 'GWANGJU', 'JEONNAM', 'JEONBUK')   COMMENT '지역 코드',
    ...
    reservation_date      DATE                                                  COMMENT '예약 날짜',
    reservation_type      ENUM ('DAY_USE', 'STAY')                              COMMENT '예약 유형',
    ...
    reservation_status    VARCHAR(255) DEFAULT 'NOT_REGISTERED' NOT NULL        COMMENT '예약 상태'
);
